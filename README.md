...
6. For manual setup, add JWT_SECRET and ENCRYPTION_KEY in Render's Environment page. ENCRYPTION_KEY must be a base64 value that decodes to exactly 32 bytes. The Blueprint generates both. Keep existing values if updating a running service, or saved PAN numbers may become unreadable. Never put secrets in GitHub.
