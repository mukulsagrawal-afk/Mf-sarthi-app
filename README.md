...
- The bundled catalog contains all 14,353 active scheme variants across 54 AMCs in AMFI's official NAV master at the 20 September 2026 sync. Direct, Regular, Growth and IDCW options are individually searchable.
- The official-only holdings snapshot contains 532 underlying portfolios, 36,018 disclosed security rows and 1,451 scheme variants mapped to an official AMC portfolio. It includes consolidated official workbooks from SBI and Nippon India and improved mapping for coded disclosure sheets. All bundled holdings imports carry the 31 August 2026 date. Every remaining AMFI scheme stays visible with an “Official file pending” status instead of disappearing or receiving substituted data.
- Holdings Intelligence uses only AMFI's portfolio-disclosure registry and files downloaded from official AMC disclosure pages. No third-party holdings aggregator is included in the seed or monthly importer.
...
- The monthly administrator update first refreshes the complete scheme universe from AMFI's official `NAVAll.txt`, then checks the official AMC disclosure registry and AMC files. The administrator screen tracks every AMC, including AMCs whose disclosure URL is missing or whose page can no longer be read automatically. An official Excel upload is the fallback for those sources. Coverage gaps are shown instead of being silently treated as zero holdings.
- SBI Mutual Fund and Nippon India Mutual Fund use their official monthly download endpoints because their disclosure pages generate file links dynamically.
...
- FolioXpert calculates pairwise portfolio overlap from verified underlying securities. For each shared security it takes the smaller of the two disclosed portfolio weights, then sums those common weights. It shows the highest-overlap pair, average pair overlap and largest shared positions; it does not treat overlap as return correlation.
