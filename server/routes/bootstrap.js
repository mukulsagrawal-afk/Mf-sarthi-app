...
router.get('/', (req, res) => {
  ...
  const meetings = db.prepare(`
    ...
    FROM meetings m LEFT JOIN clients c ON c.id = m.client_id LEFT JOIN leads l ON l.id = m.lead_id
    WHERE m.user_id = ? ORDER BY m.scheduled_at ASC
  `).all(uid);
  const followups = db.prepare(`
    SELECT f.*, c.name as client_name, l.name as lead_name
    FROM followups f LEFT JOIN clients c ON c.id = f.client_id LEFT JOIN leads l ON l.id = f.lead_id
